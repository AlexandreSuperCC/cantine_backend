...
public interface IPlatDao extends JpaRepository<PlatDomain,Integer> {
    ...
    * @author yuan.cao@utbm.fr
    **/
    @Query(nativeQuery=true, value ="select m.id,m.name,m.type,CAST(m.rate AS DECIMAL(10,2)) rate_cast,m.content,m.amount,m.day,m.imgurl,m.ts from menu m where m.dr=0 and cid = :cid order by m.day,m.type,rate_cast desc ")
